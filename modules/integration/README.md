...
Zabbix Item 按 `extensions/mappings` 中的映射文档写成指标目录和来源绑定。执行代码不保存 item key 到 metric key 的常量。History 已有只读增量接口：来源实际数值类型、clock/ns 分页、绑定转换、权限和有界预算。未实现 History 持久采集、Trigger、Preview/Replay、可视化编辑器和 Integration Copilot。参见 [ADR-016](../../docs/adr/016-bounded-history-read.md)。
