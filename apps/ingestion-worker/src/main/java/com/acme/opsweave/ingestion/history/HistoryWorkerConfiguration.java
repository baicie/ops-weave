...
@EnableConfigurationProperties(HistoryWorkerProperties.class)
@EnableScheduling
public class HistoryWorkerConfiguration {
    @Bean(destroyMethod = "close")
    HikariDataSource historyDataSource(HistoryWorkerProperties p) {
        HistoryCheckpoint.initial(p.initialFrom());
        new HistoryPollPolicy(p.stepSeconds(), p.overlapSeconds(), p.delaySeconds(), p.maxPages());
        if (!"tenant-demo".equals(p.tenant()) || p.pollMillis() < 1000 || p.pollMillis() > 3600000
    ...
    @Bean
    IngestMetricHistoryUseCase ingestMetricHistoryUseCase(HistoryWorkerProperties p, HikariDataSource historyDataSource) {
        var source = new PlatformHistoryReader(URI.create(p.platformUrl()), p.platformToken(), p.expectedDataMode());
